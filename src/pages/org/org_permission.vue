<template>
  <div class="panel">
    ...
    <div class="panel-body" v-loading="load_data" element-loading-text="加载中">
      ...
      <el-transfer filterable :filter-method="filterMethod" filter-placeholder="角色编号/角色名称" :titles="['当前机构角色', '未授予角色']" @change="handleChange" v-model="unSelectedRoles" :data="orgPermissionData">
...
export default {
  data() {
    let initData = {
      ...
      //code: [string]当前机构的code
      //orgPermissionData: [array<json>]当前机构的角色信息和待选角色的信息
      selectedRoles: [], //[array<string>] 穿梭框左侧已选角色的ID
      unSelectedRoles: [] //[array<string>] 穿梭框右侧未选角色的ID
    };

    initData.code = this.$route.params.code;
    initData.orgPermissionData = [];
  ...
  methods: {
    getData(initData) {
      ...
      this.$fetch.api_org
        ...
        .then(({ data }) => {
          ...
          //orgPermissionData:所有数据JSON，即后台传递给前台的数据
          initData.orgPermissionData = data;
          ...
          this.$fetch.api_org
            ...
            .then(({ data }) => {
              ...
              let fullorgPermissionData = initData.orgPermissionData;
              this.orgPermissionData = fullorgPermissionData.concat(data);
    ...
    handleChange(value, direction, movedKeys) {
      for (let i in this.orgPermissionData) {
        let role = this.orgPermissionData[i];
        if (movedKeys.includes(role.key)) {
          if ("left" === direction) {
            this.selectedRoles.push(role.key);
          } else {
            this.selectedRoles.splice(this.selectedRoles.indexOf(role.key), 1);
          }
        }
      }
    },
